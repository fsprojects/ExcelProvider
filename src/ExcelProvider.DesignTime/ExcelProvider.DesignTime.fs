...
module internal Helpers =
    ...
    let propertyImplementation columnIndex columnName (value: obj) =
        match value with
        | :? float ->
            typeof<double>, (fun row -> <@@ (%%row: Row).TryGetNullableValue<double> columnIndex columnName @@>)
        | :? bool -> typeof<bool>, (fun row -> <@@ (%%row: Row).TryGetNullableValue<bool> columnIndex columnName @@>)
        | :? DateTime ->
            typeof<DateTime>, (fun row -> <@@ (%%row: Row).TryGetNullableValue<DateTime> columnIndex columnName @@>)
                          ...
                          let getter =
                              (fun row ->
                                  <@@
                                      ...
                                      if String.IsNullOrEmpty value then null else value
                                  @@>)
                              |> singleItemOrFail
...
type public ExcelProvider(cfg: TypeProviderConfig) as this =
    inherit
        TypeProviderForNamespaces(
            ...
            assemblyReplacementMap = [ ("ExcelProvider.DesignTime", "ExcelProvider.Runtime") ]
        )
    ...
    let excelFileProvidedType =
        ProvidedTypeDefinition(executingAssembly, rootNamespace, "ExcelFile", Some(typeof<ExcelFileInternal>))
    ...
    let buildTypes (typeName: string) (args: obj[]) =
        ...
        let ProvidedTypeDefinitionExcelCall (filename, sheetname, range, hasheaders, forcestring) =
            ...
            let providedExcelFileType =
                ProvidedTypeDefinition(executingAssembly, rootNamespace, typeName, Some(typeof<ExcelFileInternal>))

            // add a parameterless constructor which loads the file that was used to define the schema
            providedExcelFileType.AddMember(
                ProvidedConstructor(
                    [],
                    invokeCode =
                        emptyListOrFail (fun () ->
                            <@@ ExcelFileInternal(resolvedFilename, sheetname, range, hasheaders) @@>)
                )
            )
            ...
            providedExcelFileType.AddMember(
                ProvidedConstructor(
                    ...
                    invokeCode =
                        singleItemOrFail (fun filename ->
                            <@@ ExcelFileInternal(%%filename, sheetname, range, hasheaders) @@>)
                )
            )
            ...
            providedExcelFileType.AddMember(
                ProvidedConstructor(
                    [ ProvidedParameter("filename", typeof<string>)
                      ProvidedParameter("sheetname", typeof<string>) ],
                    invokeCode =
                        twoItemsOrFail (fun fileName sheetname ->
                            <@@ ExcelFileInternal(%%fileName, %%sheetname, range, hasheaders) @@>)
                )
            )
            ...
            providedExcelFileType.AddMember(
                ProvidedConstructor(
                    [ ProvidedParameter("stream", typeof<Stream>)
                      ProvidedParameter("format", typeof<ExcelFormat>) ],
                    invokeCode =
                        twoItemsOrFail (fun stream format ->
                            <@@ ExcelFileInternal(%%stream, %%format, sheetname, range, hasheaders) @@>)
                )
            )
            ...
            providedExcelFileType.AddMember(
                ProvidedConstructor(
                    [ ProvidedParameter("stream", typeof<Stream>)
                      ...
                      ProvidedParameter("sheetname", typeof<string>) ],
                    invokeCode =
                        threeItemsOrFail (fun stream format sheetname ->
                            <@@ ExcelFileInternal(%%stream, %%format, %%sheetname, range, hasheaders) @@>)
                )
            )
            ...
            providedExcelFileType.AddMember(
                ProvidedProperty(
                    "Data",
                    typedefof<seq<_>>.MakeGenericType(providedRowType),
                    getterCode = singleItemOrFail (fun excFile -> <@@ (%%excFile: ExcelFileInternal).Data @@>)
                )
            )
